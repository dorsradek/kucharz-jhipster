...
class PrzepisProduktGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrzepisProdukt entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all przepisProdukts")
            .get("/api/przepisProdukts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new przepisProdukt")
            .post("/api/przepisProdukts")
            .headers(headers_http_authenticated)
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created przepisProdukt")
                .get("${new_przepisProdukt_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created przepisProdukt")
